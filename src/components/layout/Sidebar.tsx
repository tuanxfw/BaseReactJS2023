...
import { getSidebarStatus, getSidebarData } from "@redux/selectors/sidebarSelectors";
...
const Sidebar = () => {
  ...
  const genMenuData = (): MenuProps["items"] => {
    ...
    const menuData = localStoreUtil.getData("menu");
  ...
  const onOpenChange = (keys: string[]) => {
    const latestOpenKey = _.find(keys, (key) => state.openKeys.indexOf(key) === -1);
    ...
    if (!_.some(roots, (item) => item["id"] === latestOpenKey)) {
      ...
      dispatch(setSidebarData({ openKeys: latestOpenKey ? [latestOpenKey] : [] }));
    }
  };
  //#endregion
      ...
      <Drawer
        title={
          <div className="title-sidebar">
            <div className="logo">
              <img src={`${AppConfig.VITE_PUBLIC_URL}/images/logo.png`} alt="" />
            </div>
