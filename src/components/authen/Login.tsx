...
import { keycloakUtil, localStoreUtil } from "@utils/commonUtil";
import { useLoginSSO } from "@hooks/fetch/useAuth";
...
const Login = () => {
  ...
  //#region Method
  const checkLogin = _.debounce(async () => {
    const url = window.location;
    const urlParam = queryString.parse(url.search);

    if (_.isEmpty(urlParam.code)) {
      window.location.href = keycloakUtil.getUrlLoginSSO();
    } else {
      loginSSO(_.toString(urlParam.code));
    }
  }, 100);

  const loginSSO = async (code: string) => {
    try {
      const tokenData: any = await signIn.mutateAsync(code);
