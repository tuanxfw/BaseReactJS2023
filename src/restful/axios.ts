...
function logout(path = LOGOUT, data = {}) {
...
const genApiUrl = (path: string) => {
    ...
    let url = AppConfig.VITE_BUSINESS_URL_API.replace(/\/$/, "") + "/" + path.replace(/^\//, "");
    return url;
};
...
const handleRestApi = (config: ConfigRequest) => (res: any) => {
    ...
    let result = _.get({ res }, config.queryRes as string, {});
    ...
    if (error?.response?.status === 401) {
        if (!localStoreUtil.checkLoginLocal()) {
            if (_.get({ error }, "error.response.request.responseURL", "").includes(keycloakUtil.getUrlLogin())) {
                showMessage({ type: "error", message: i18n.t("login:infoLoginiIsValid") });
            }
            else {
                alert(i18n.t("common:errors.sessionLoginIsExpired"));
                logout();
            ...
            let tokenData = await login(keycloakUtil.getUrlLogin(), params);
            ...
            let res = await instance(error.config);
